...
def actor_out_of_work
  ...
  Actor.select(:name).joins("LEFT OUTER JOIN castings on castings.actor_id = actors.id").where(castings: { movie_id: nil }).count
end
...
def longest_career
  ...
  # their career.
  Actor.select(:name, :id, "MAX(movies.yr) - MIN(movies.yr) AS career").joins(:movies).order("career DESC, name").group(:id).limit(3)
end
