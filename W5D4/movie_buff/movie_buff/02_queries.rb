...
def most_supportive
  ...
  Actor.select(:id, :name, "COUNT(castings.actor_id) as roles").joins(:castings).where.not(castings: { ord: 1 }).group("actors.id").order("count(castings.actor_id) DESC").limit(2)
end
