...
def andrews_films_and_leads
  ...
  FROM (SELECT movies.*
        ...
        WHERE actors.name = 'Julie Andrews') AS m
  JOIN (SELECT actors.*, castings.movie_id AS movieid
        FROM actors
        JOIN castings
        ON castings.actor_id = actors.id
        WHERE castings.ord = 1) as a
  ON m.id = a.movieid
  ORDER BY m.title
...
def colleagues_of_garfunkel
  ...
  FROM (SELECT movies.*
          ...
          JOIN actors
            ON actors.id = castings.actor_id
         WHERE actors.name = 'Art Garfunkel') AS m
  JOIN (SELECT actors.*, castings.movie_id
          FROM actors
          JOIN castings
            ON castings.actor_id = actors.id
         WHERE actors.name != 'Art Garfunkel') as a
    ON m.id = a.movie_id;
  SQL
end
